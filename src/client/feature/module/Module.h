...
public:
    ...
    explicit Module(std::string const& name, std::wstring const& displayName, std::wstring const& description,
                    Category category, int keybind = 0, bool hud = false, bool visible = true)
    ...
    explicit Module(std::string const& name, LocalizedString const& displayName, LocalizedString const& description,
                    Category category, int keybind = 0, bool hud = false, bool visible = true)
        : Module(name, displayName.value(), description.value(), category, keybind, hud, visible) {
        setDisplayName(displayName);
        setDescription(description);
    }
    explicit Module(std::string const& name, LocalizedString const& displayName, std::wstring const& description,
                    Category category, int keybind = 0, bool hud = false, bool visible = true)
        : Module(name, displayName.value(), description, category, keybind, hud, visible) {
        setDisplayName(displayName);
    }
    explicit Module(std::string const& name, std::wstring const& displayName, LocalizedString const& description,
                    Category category, int keybind = 0, bool hud = false, bool visible = true)
        : Module(name, displayName, description.value(), category, keybind, hud, visible) {
        setDescription(description);
    }
    ...
    virtual ~Module() { Eventing::get().unlisten(this); }
    ...
    std::shared_ptr<Setting> addSetting(std::string const& internalName, std::wstring const& displayName,
                                        std::wstring const& desc, ValueType& val,
                                        Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addSetting(std::string const& internalName, LocalizedString const& displayName,
                                        std::wstring const& desc, ValueType& val,
                                        Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addSetting(std::string const& internalName, std::wstring const& displayName,
                                        LocalizedString const& desc, ValueType& val,
                                        Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addSetting(std::string const& internalName, LocalizedString const& displayName,
                                        LocalizedString const& desc, ValueType& val,
                                        Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, std::wstring const& displayName,
                                            std::wstring const& desc, EnumData& dat,
                                            Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, LocalizedString const& displayName,
                                            std::wstring const& desc, EnumData& dat,
                                            Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, std::wstring const& displayName,
    ...
    std::shared_ptr<Setting> addEnumSetting(std::string const& internalName, LocalizedString const& displayName,
                                            LocalizedString const& desc, EnumData& dat,
                                            Setting::Condition condition = Setting::Condition());
    std::shared_ptr<Setting> addSliderSetting(std::string const& internalName, std::wstring const& displayName,
                                              std::wstring const& desc, ValueType& val, ValueType min, ValueType max,
